...
> 由于此网站的主题插件的限制，本文的所有unique语句无法直接复制。如果你使用的是Edge浏览器，可以使用[Allow copy](https://microsoftedge.microsoft.com/addons/detail/allow-copy/ijekdgpkaednghahipmbbacnabibojeh)插件来强制复制；对于其他浏览器，自行搜索相关插件。
...
<details>
<summary><span title="数字，通常带有 + 或 - 号，例如 `+25`（此类参数通常后跟 '%'，但 '%' 不属于值的一部分）">[relativeAmount]</span>% City-State 影响力 degradation</summary>
...
<details>
<summary>可以与城邦进行贸易任务，给予大量金币和 <span title="整数，可能有正负号，例如 `2`、`+13` 或 `-3`">[amount]</span> 影响力</summary>
...
<details>
<summary>只能攻击 <span title="指示战斗者，可以是单位或城市（轰炸时）。必须是 `City` 或 `mapUnitFilter`">[combatantFilter]</span> units</summary>
...
<details>
<summary>只能攻击 <span title="任何可以在 improvementFilter 或 terrainFilter 中使用的内容都可以在这里使用，加上 'unimproved'（未改良）">[tileFilter]</span> tiles</summary>
...
<details>
<summary>爆炸半径 <span title="整数，可能有正负号，例如 `2`、`+13` 或 `-3`">[amount]</span></summary>
...
<details>
<summary>强度为...的核武器 <span title="整数，可能有正负号，例如 `2`、`+13` 或 `-3`">[amount]</span></summary>
...
<details>
<summary>May enter foreign tiles without open borders, 但失去 <span title="整数，可能有正负号，例如 `2`、`+13` 或 `-3`">[amount]</span> 宗教力量，每回合结束时</summary>

示例："May enter foreign tiles without open borders, 但失去 [3] 宗教力量，每回合结束时"
